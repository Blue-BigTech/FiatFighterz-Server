...
namespace WebServer
{
    public class WebServer
    {
...
#else
        private const string Prefix = "http://api.fiatfighterz.com/";
#endif
        private BlockChainClient bcc;
        private const string ServerName = "Web";

        /// <summary>
        /// Rsa decryption used to read secrets
        /// </summary>
        private static Rsa rsa = new Rsa(false);
        ...
        public WebServer()
        {
            ...
            listener.AddHandler("v1/account/getownednfts", HandleGetOwnedNFTs);
            listener.AddHandler("v1/account/sellitem", HandleSellItem);
            listener.AddHandler("v1/account/generatemessage", HandleGenerateMessage);
            listener.AddHandler("v1/account/tswlogin", HandleTwoStageVerificationLogin);
            listener.AddHandler("v1/account/tswdescribe", HandleTwoStageVerificationToken);
            ...
            listener.AddHandler("v1/metadata/token", HandleTokenMetaData);
            listener.AddHandler("v1/metadata/file", HandleTokenMetaFile);

        }

        private async Task<object> HandleTokenMetaFile(HttpListenerContext context, NameValueCollection query)
        {
            HttpListenerRequest req = context.Request;
            Console.WriteLine("Requested Token Meta:" + req.Url);
            var fileName = req.Url.LocalPath.Replace("/v1/metadata/file/" , "");
            try
            {
                byte[] fileData = File.ReadAllBytes("./Files/" + fileName);
                return fileData;
            }
            catch (Exception ex)
            {
        ...
        private async Task<object> HandleTokenMetaData(HttpListenerContext context, NameValueCollection query)
        {
            HttpListenerRequest req = context.Request;
            Console.WriteLine("Requested Token Meta:" + req.Url);
            var tokenIDString = req.Url.LocalPath.Replace("/v1/metadata/token/", "");
            try
            {
                var tokenID = ulong.Parse(tokenIDString);
                var metaObject = await Database.GetNFTMetaDataFromTokenID(tokenID);
                if (metaObject != null)
                {
                    metaObject.image = Prefix + "v1/metadata/file/" + metaObject.image;
                    string result = JsonConvert.SerializeObject(metaObject);
                    result = result.Replace("\"max_value\":null,", "");
                    return result;
                }
                return "{result:error}";
            }
        ...
        public async void Start()
        {
            ...
            //await bcc.DetermineItemOwnership();

            //await bcc.DetermineItemOwnership("17035972401136905196", "2759035388156887233", "11154852578056925872");
            /*
            Distributor distributor = new Distributor();

            var task = Task.Run(async () => await distributor.SendToken("",1));
        ...
        private async Task<object> HandleLogin(HttpListenerContext context, NameValueCollection query)
        {
            ...
            var hash = query["hash"];
            var cryptoAddress = query["cryptoAddress"];

            if (AnyNull(email, hash, cryptoAddress))
            {
            ...
            cryptoAddress = DecryptString(cryptoAddress);

            
           
            return await Database.WebLogin(email, hash, cryptoAddress);
        }
        ...
        {
            Log.Write("Received register request.");
            ...
            Console.WriteLine("second test email: " + email + " --------------");
            Console.WriteLine("hash: " + hash + " -------------");
             Console.WriteLine("crypptoAddress: " + cryptoAddress);

            if (AnyNull(email, hash))
            {
                Log.Write("Blank email or hash");
                return new WebRegisterResponse(WebRegisterResult.InvalidRequest, null);
            }

            email = DecryptString(email);
            Console.WriteLine("lmaDrecrypted email to " + email);
            hash = DecryptString(hash);
            cryptoAddress = DecryptString(cryptoAddress);

            var createResponse = await Database.CreateAccount(email, hash, cryptoAddress);
        ...
        private async Task<object> HandleVerified(HttpListenerContext context, NameValueCollection query)
        {
            ...
            /*
             * captcha verification no longer works
            ...
            if (version != null)
            {
                if (NetConstants.BuildAhead(version))
                {
                    ...
                    return new WebServerListResponse(WebServerListResult.Success, new WebServerInfo[] {
                        new WebServerInfo("eatmyass", "71.195.254.8", "71.195.254.8", ServerStatus.Normal)
                    });
                }
            }
        ...
        private async Task<object> HandleCreateCharacter(HttpListenerContext context, NameValueCollection query)
        {
            ...
            //Database.CreateCharacter(account, new ushort());
            var orderIdString = query["orderId"];
            var accountIdString = query["accountId"];
            return null;
        }

        public async Task<object> HandleSellItem(HttpListenerContext context, NameValueCollection query)
        {
            var token = DecryptString(query["token"]);
            var accountId = DecryptString(query["accountId"]);
            var characterId = DecryptString(query["characterId"]);
            var itemId = DecryptString(query["itemId"]);
        ...
        // old get owned nft method
        /*
                outFilterRequest += i == 0 ? (string.Format(filterRequest, ownedNFT.token_id.ToString()))
                                            : ( filterRequestComma + string.Format(filterRequest, ownedNFT.token_id.ToString()) );
                expAttributeValues.Add(":" + ownedNFT.token_id, new AttributeValue { S = ownedNFT.token_id.ToString() });
                i++;*/
        //break;// is it too large ?

        /*
         * string filterRequest = "token_id = :{0}";
            string outFilterRequest = "";
            string filterRequestComma = " OR ";
        foreach (string f in item["itemIds"].NS)
        {
            Console.WriteLine("thisthisthsi" +f);
            itemIds[a] = ulong.Parse(f);
            a++;
        }

        var serverItems = new Item[4];
        var itemIds = new ulong[4];
        for (int j = 0; j < itemIds.Length && j < serverItems.Length; j++)
        {
            var itemId = itemIds[j];
            if (itemId == 0) continue;

            var ressponse = await ServerItem.Get(itemId);
            switch (ressponse.result)
            {
                case ServerItemGetResult.Success:
                    serverItems[j] = ressponse.item.itemData;
                    break;
        ...
        webCharacter.Add(
            ...
            {
                id = ulong.Parse(item["id"].N),
                type = ushort.Parse(item["type"].N),
                skin = ushort.Parse(item["skin"].N),
                equips = serverItems,
                metadata = item["metadata"].S
            }
        );
            //characters.Add(characterResponse.item);
            //webCharacter.Add( await characterResponse.item.GetWebInfo() );

        */



        public async Task<object> HandleGetOwnedNFTs(HttpListenerContext context, NameValueCollection query)
        {
            DateTime dt = DateTime.UtcNow;
            var token = DecryptString(query["token"]);
            var signedmessage = DecryptString(query["signedMessage"]);
            var message = signedmessage.Split("_FF_")[1];
            var signed = signedmessage.Split("_FF_")[0];
            var tokenResponse = await TokenLogin.Get(token); // retrieve token login infos
            var accountResponse = await Account.Get(tokenResponse.item.accountId); // retrieve account
            var cryptoAddress = CryptoVerify.VerifySignedMessage(message,signed);
            var accountIdString = DecryptString(query["accountId"]);
            var cryptoVerify = await Database.WebCryptoVerfy(token, cryptoAddress);

            Console.WriteLine("Verified database account and wallet address: " + (DateTime.UtcNow - dt) );
            if (!cryptoVerify)
            {
                Console.WriteLine("bad crypto verify");
            ...
            WebGetOwnedNFTsResponse ownedNFTsResponse = await bcc.DetermineOwnership(accountIdString, cryptoAddress);
            Console.WriteLine("block chain client determine ownership: " + (DateTime.UtcNow - dt));
            var account = accountResponse.item;
            if (ownedNFTsResponse.result == WebGetOwnedNFTsResult.Success)
            {
                List<WebCharacterInfo> webcharacers = new List<WebCharacterInfo>();

                var keys = new List<Dictionary<string, AttributeValue>>();
                foreach (WebGetOwnedNFTsInfo ownedNFT in ownedNFTsResponse.OwnedNfts)
                {
                    keys.Add(new Dictionary<string, AttributeValue> {
                        { "id", new AttributeValue { 
                            N = ownedNFT.token_id.ToString() } 
                        }
                    });
                }
                ...
                {
                    RequestItems = new Dictionary<string, KeysAndAttributes>() { { Database.Table_Characters, new KeysAndAttributes { Keys = keys } } }
                };

                var wecharacterinfos = await Database.GetCharacterInfoBatch(batchrequest);
                var webCharacters = webcharacers.ToArray();
                //var webCharacters = new WebCharacterInfo[characters.Count];
                Console.WriteLine(webCharacters.Length + " -- " + account.cryptoAddress + " -- " + account.playerName);
                /*
                for (int x = 0; x < webCharacters.Length; x++)
                    webCharacters[x] = await characters[x].GetWebInfo();
                */
                Console.WriteLine("web info for characters " + (DateTime.UtcNow - dt));

                var response = new WebDescribeResponse(
            ...
            {
                var response = new WebDescribeResponse(
                    WebDescribeResult.InvalidRequest,
                    account.id,
                    account.premiumCurrency,
                    account.playerName,
                    account.email,
        ...
        public async Task<object> HandleGenerateMessage(HttpListenerContext context, NameValueCollection query)
        {
            if (!rateLimiter.CanRequest(context.Request.RemoteEndPoint.Address))
            {
                return new WebMessageResponse(WebMessageResult.RateLimitReached);
            }
            Console.WriteLine("generateMessage");
            string message = CryptoVerify.GenerateMessage(aes);

            Console.WriteLine(aes.Decrypt(message));
            return new WebMessageResponse(WebMessageResult.Success, message);
        }

        public async Task<object> HandleVerifyMessage(HttpListenerContext context, NameValueCollection query)
        {
            if (!rateLimiter.CanRequest(context.Request.RemoteEndPoint.Address))
            {
                return new WebDescribeResponse(WebDescribeResult.RateLimitReached);
            }
            var originMessage = DecryptString(query["originMessage"]);
            var signedMessage = DecryptString(query["signedMessage"]);

            Console.WriteLine(originMessage + " " + signedMessage + "veryify message");

            var address = CryptoVerify.VerifySignedMessage(originMessage, signedMessage);

            if (address == "")
            {
                return new WebSignedMessageResponse(WebSignedMessageResult.InvalidSignature);
            }
            return new WebSignedMessageResponse(WebSignedMessageResult.Success, signedMessage, originMessage);
        }


        private async Task<object> HandleTwoStageVerificationToken(HttpListenerContext context, NameValueCollection query)
        {
            if (!rateLimiter.CanRequest(context.Request.RemoteEndPoint.Address))
            {
                return new WebDescribeResponse(WebDescribeResult.RateLimitReached);
            }
            var originMessage = query["originMessage"];
            var signedMessage = query["signedMessage"];
            var token = query["token"];
            var version = query["version"];



            if (AnyNull(token, version, originMessage, signedMessage))
            {
                return new WebDescribeResponse(WebDescribeResult.InvalidRequest);
            }

            originMessage = DecryptString(originMessage);
            signedMessage = DecryptString(signedMessage);
            token = DecryptString(token);
            ...
            if (describe.result == WebDescribeResult.Success)
                describe.servers = serverList.infos;
            var accountResponse = await Account.Get(describe.accountId); // retrieve account

            if (accountResponse.item.cryptoAddress == CryptoVerify.VerifySignedMessage(originMessage, signedMessage))
            {
                
                //Console.WriteLine(CryptoVerify.VerifySignedMessage(originMessage, signedMessage));
                return describe;
            }
        ...
        private async Task<object> HandleTwoStageVerificationLogin(HttpListenerContext context, NameValueCollection query)
        {
            if (!rateLimiter.CanRequest(context.Request.RemoteEndPoint.Address))
            {
                return new WebLoginResponse(WebLoginResult.RateLimitExceeded, null);
            ...
            if (AnyNull(email, hash, originMessage, signedMessage))
            {
                return new WebLoginResponse(WebLoginResult.InvalidRequest, null);
            }
            ...
            if (CryptoVerify.ValidateMessageTime(aes, originMessage)) ;
            var cryptoAddress = CryptoVerify.VerifySignedMessage(originMessage, signedMessage);
            Console.WriteLine(email + " " + hash + " " + cryptoAddress);
            return await Database.WebLogin(email, hash, cryptoAddress);
        }
