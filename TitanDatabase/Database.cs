...
namespace TitanDatabase
{
    ...
    {
        //public const string Rsa_Private_Key = "";
        public const string Rsa_Private_Key = "";
        ...
        public static ManualResetEvent Initialize()
        {
            ...
            var resetEvent = new ManualResetEvent(false);// return this to true when done
            if (client != null)
            {
                resetEvent.Set();
            }
            else if (CreateClient(local))        // hard coded in for testing a local database replace this with local when finished
            {
        ...
        private static bool CreateClient(bool local)
        {
            if (local)
            {
                ...
                // TODO https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/DynamoDBLocal.DownloadingAndRunning.html
                ...
                try { client = new AmazonDynamoDBClient(new BasicAWSCredentials(AwsConstants.Key, AwsConstants.Secret), ddbConfig); } // TODO change credentials
                catch (Exception ex)
                {
        ...
        public static string Table_Token_Transactions = ModularProgram.manifest.Value("dbPrefix", "") + "titan_token_transactions";
        ...
        public static string Table_NFT_Metadata = ModularProgram.manifest.Value("dbPrefix", "") + "nft_metadata";
        ...
        private static async void CreateTables(ManualResetEvent resetEvent)
        {
            var tableCreators = new Dictionary<string, Func<Task<bool>>>()
            {
                ...
                { Table_Token_Transactions, CreateTokenTransactionsTable},
            };
        ...
        private static async Task<bool> CreateTokenTransactionsTable()
        {
            var request = new CreateTableRequest()
            {
                TableName = Table_Token_Transactions,
                AttributeDefinitions = new List<AttributeDefinition>
                {
                    new AttributeDefinition("id", ScalarAttributeType.N)
                },
        ...
        public static async Task<CreateAccountResponse> CreateAccount(string email, string hash, string cryptoAddress)
        {
            ulong id = 0;
        ...
        public static async Task<WebCharacterInfo[]> GetCharacterInfoBatch(BatchGetItemRequest batchrequest)
        {
            var response = await client.BatchGetItemAsync(batchrequest);
            var webcharinfos = new List<WebCharacterInfo>();
            var batchgetitems = new BatchGetItemRequest();
            var itemidmapper = new List<Dictionary<ulong, Item>[]>();
            var keys = new List<Dictionary<string, AttributeValue>>();
            var indexmap = new Dictionary<string, int[]>();
            int index = 0;
            foreach (var res in response.Responses[Database.Table_Characters])
            {
                var serverItems = new Item[4];
                var l = res["itemIds"].L;
                var itemslotdict = new Dictionary<ulong, Item>[4];

                for (int i = 0; i < l.Count && i < 4; i++)
                {
                    var itemId = ulong.Parse(l[i].N);
                    if (itemId == 0) continue;
                    itemslotdict[i] = new Dictionary<ulong, Item> { { itemId, new Item()} };
                    keys.Add(new Dictionary<string, AttributeValue> {
                        
                            { "id", new AttributeValue{ N = l[i].N } }
                    });
                    indexmap[l[i].N] = new int[2]{ index , i };
                    Console.WriteLine(itemId);
                    /*
                     * non batch method
                    var ress = await ServerItem.Get(itemId);
                    if (ress.result == ServerItemGetResult.Success)
                    {
                        serverItems[i] = ress.item.itemData;
                    }
                    Console.WriteLine(l[i].N);*/
                }
                itemidmapper.Add(itemslotdict);
                WebMetaData metaData = GenerateMetadata(res["metadata"].S, ushort.Parse(res["level"].N), ushort.Parse(res["type"].N));
                webcharinfos.Add(
                    new WebCharacterInfo
                    {
                        id = ulong.Parse(res["id"].N),
                        type = ushort.Parse(res["type"].N),
                        skin = ushort.Parse(res["skin"].N),
                        equips = serverItems,
            ...
            batchgetitems.RequestItems = new Dictionary<string, KeysAndAttributes>() { { Database.Table_Items, new KeysAndAttributes { Keys = keys} } };
            var batchResponse = await client.BatchGetItemAsync(batchgetitems);
            Console.WriteLine(batchResponse.Responses[Database.Table_Items].Count);
            foreach (var res in batchResponse.Responses[Database.Table_Items]) {
                var obj = new ServerItem();
                obj.Read(new ItemReader(res));
                obj.StoreValues(res);
                webcharinfos[indexmap[res["id"].N][0]].equips[indexmap[res["id"].N][1]] = obj.itemData;
                /*
                int ind = indexmap[res["id"].N][0];
                int innerind = indexmap[res["id"].N][1];*/
            }

            return webcharinfos.ToArray();
        } 

        public static WebMetaData GenerateMetadata(string metaString, ushort level, ushort type)
        {
            CharacterInfo objectInfo = (CharacterInfo)GameData.objects[type];
            WebMetaData metaDataObject = JsonConvert.DeserializeObject<WebMetaData>(metaString);
            List<trait> originalTraits = metaDataObject.attributes.ToList();
            foreach (var stats in objectInfo.stats)
            {
                int value = stats.Value.baseValue;
                for (int l = 0; l < level; l++)
                {
                    if (l < stats.Value.increaseLoop.Length)
                    {
                        value += stats.Value.increaseLoop[l];
                    }

                }
                trait t = new trait() { trait_type = stats.Value.type.ToString(), value = value.ToString(), max_value = stats.Value.maxValue.ToString() };
                originalTraits.Add(t);
            }
            metaDataObject.attributes = originalTraits.ToArray();
            //
            return metaDataObject;
        }

        public static async Task<WebMetaData> GetNFTMetaDataFromTokenID(ulong tokenID)
        {
            Dictionary<string, AttributeValue> keys = new Dictionary<string, AttributeValue>();
            keys.Add("id", new AttributeValue { N = tokenID.ToString() });
            GetItemResponse characterData = await client.GetItemAsync(Database.Table_Characters, keys);
            
            if (characterData != null)
            {
                try
                {
                    string metaString = characterData.Item["metadata"].S;
                    ushort level = ushort.Parse(characterData.Item["level"].N);
                    ushort type = ushort.Parse(characterData.Item["type"].N);
                    return GenerateMetadata(metaString, level, type);
                }
                catch (Exception ex)
                {
                    return null;
        ...
        public static async Task<CreateCharacterResponse> CreateCharacterNFT(WebGetOwnedNFTsInfo nftinfo, ulong gen_id)
        {
            NFTmetadata metadata = new NFTmetadata(nftinfo.metadata);
            ushort aatype = 0x1;
            Console.WriteLine(metadata.Class);
            switch (metadata.Class.ToLower())     // mage should be named wizard change this in the next mint 
            {
                case "wizard":
                    aatype = 0xc; // Fiat Class
                    break;
                case "archer":
                    aatype = 0x1; // ranger
                    break;
            ...
            if (!GameData.objects.TryGetValue(aatype, out var info) || !(info is CharacterInfo charInfo))
                return new CreateCharacterResponse(null, CreateCharacterResult.InvalidCharacterType);

            Console.WriteLine(aatype + " " + charInfo.defaultItems[0]);

            // this is what the metadata.Character value will be in the next mint
            var skinname = (metadata.Class+metadata.DNA).ToString().ToLower();

            if (skinname.Contains("preist")) {
                skinname = skinname.Replace("preist", "priest");
            }
            if (skinname.Contains("cyborg"))
            {
                skinname = skinname.Replace("cyborg", "cyclops");
            }
            GameObjectInfo skinInfo = GameData.GetObjectByName(skinname+ " Skin");
            if (skinInfo == null) { Console.WriteLine("Could Not Find: " + skinname);  }
            ...
            {
                ulong id = gen_id;
                //ulong id = GenerateUInt64Id();      // where it creates that long string for the character id
                if (id == 0)
                {
                    idFailed = true;
                    continue;
            ...
            for (int i = 0; i < character.itemIds.Count; i++)
            {
                ...
                //Console.WriteLine("create item request on " + defaultItem + " " + character.id);
                ...
                character.itemIds[0] = createResponse.item.id;
                var characterResponse = await character.Put();
                if (characterResponse.result != RequestResult.Success)
                {
                    switch (characterResponse.result)
                    {
        ...
        public static async Task<CreateItemResponse> CreateItem(Item data, ulong containerId)
        {
            ...
            Console.WriteLine("create item request");
        ...
        public static async Task<WebLoginResponse> WebLogin(string email, string hash, string cryptoAddress)
        {
            if (string.IsNullOrWhiteSpace(email) || string.IsNullOrWhiteSpace(hash) || string.IsNullOrWhiteSpace(cryptoAddress))
                return new WebLoginResponse(WebLoginResult.InvalidRequest, null);
            ...
            // check if the account has a valid crypto address
            

            LoginResponse loginRes = await Login(emailResponse.item.accessToken, "Web");
            Log.Write(loginRes.result);

            if (!loginRes.account.cryptoAddress.Equals(cryptoAddress))  // crypto address match with a
            {
                Console.Write("Address at:{0}\n Does not match\n Address at {1}", cryptoAddress, loginRes.account.cryptoAddress);
                return new WebLoginResponse(WebLoginResult.InvalidCtyptoAddress, null);
            }
            
            return new WebLoginResponse(WebLoginResult.Success, emailResponse.item.accessToken);
        }

        public static async Task<bool> WebCryptoVerfy(string accessToken, string cryptoAddress)
        {
            if (string.IsNullOrWhiteSpace(accessToken) || string.IsNullOrWhiteSpace(cryptoAddress))
                return false;
            ...
            if (accountResponse.result != RequestResult.Success)
            {
                Log.Write("Bad Token Account");

                return false;
            }
            if (accountResponse.item.cryptoAddress.ToLower().Equals(cryptoAddress.ToLower()))
            {
                return true;
            }
            Log.Write(string.Format("sent {0} \nstored {1}", accountResponse.item.cryptoAddress.ToLower(), cryptoAddress));
            Log.Write("Crypto Address Does not Equal The One On This Account");
            return false;
        }
        ...
        public static async Task<WebDescribeResponse> WebDescribe(string accessToken)
        {
            ...
            // characters sent back need to be the ones that match the nfts 
            // im replacing the webdescribe with a new function in BlockChainClient -> HandleGetOwnedNFTs
            var characters = new List<Character>();

            foreach (var charId in account.characters)
            {
                var characterResponse = await Character.Get(charId);
                if (characterResponse.result != RequestResult.Success)      // if it fails
                {
                    switch (characterResponse.result)
                    {
                        default:
