...
entity_state(name: "Purple Wyvern", actions: {
	chase(speed: 8, min: 5)
	shoot_player(index: 1, periodMin: 1.4, periodMax: 2, amount: 2, angleGap: 20)
	wander(speed: 1, period: 1)
	shoot_player(index: 0, periodMin: 1.4, periodMax: 2, amount: 4, angleGap: 15)
})

entity_state(name: "Yellow Wyvern", actions: {
	chase(speed: 10, min: 6)
	wander(speed: 1, period: 1)
	shoot_player(index: 1, periodMin: 1, periodMax: 1.2, amount: 4, angleGap: 14)
	shoot_aoe_player(period: 2)
})

entity_state(name: "Ghost", actions: {
	chase(speed: 10, min: 6)
	wander(speed: 1, period: 1)
	shoot_player(index: 0, periodMin: 1, periodMax: 1.2, amount: 4, angleGap: 10)
	shoot_player(index: 1, periodMin: .2, periodMax: .4, amount: 1, angleGap: 0)
	shoot_player(index: 2, periodMin: .2, periodMax: .4, amount: 1, angleGap: 0)
})

entity_state(name: "Bronze Robot", actions: {
	chase(speed: 8, min: 6)
	wander(speed: 1, period: 1)
	spawn(name: "Bronze Robotlet", rate: 1, max: 8, period: 1)
	shoot_player(index: 0, periodMin: 1, periodMax: 1.2, amount: 4, angleGap: 10)
})

entity_state(name: "Bronze Robotlet", actions: {
	if_elapsed(sec: .5, false: {
		chase(speed: 25, min: 0.1)
	})
	set_flash(color: "red", duration: 2)
	shoot_player(index: 0, periodMin: 2.8, periodMax: 3, amount: 1)
	if_elapsed(sec: .75, true: {
		chase(speed: 15, min: 1)
		if_elapsed(sec: 1, true: {
		chase(speed: 15, min: 1)
			shoot_player(index: 0, amount: 8)
...
entity_state(name: "Test Boss", actions: {
	chase(speed: 8, min: 6)
	wander(speed: 1, period: 1)
	spawn(name: "Bronze Robotlet", rate: 1, max: 8, period: 1)
	shoot_player(index: 0, periodMin: 1, periodMax: 1.2, amount: 4, angleGap: 10)
