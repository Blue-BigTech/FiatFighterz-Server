...
entity_state(name: "Skeletal Archer", actions: {
	...
	wander(speed: 4, period: 0.8)
	chase(speed: 3, min: 5)
	shoot_player(index: 0, periodMin: 1.4, periodMax: 2, amount: 3, angleGap: 20)
})
entity_state(name: "Testing Dude", actions: {
	spawn(name: "Skeletal Archer", rate: 2, max: 2, radius: 3)
	hover_leader(speed: 4, minHover: 4, maxHover: 5)
	wander(speed: 2, period: 0.3)
	chase(speed: 10, min: 5)
	shoot_aoe_player(period: 2)
	shoot_player(index: 1, period: 2.2, amount: 2, angleGap: 0)
	shoot_player(index: 2, period: 1.8, amount: 4, angleGap: 15)

	shoot_player(index: 3, period: 1.8, amount: 4, angleGap: 15)
})